Pocket Budget Version 27 - iPhone backup import fix
What changed
The backup file picker no longer uses an HTML `accept` filter.
iPhone Files can therefore select encrypted `.pbe` files instead of displaying them as disabled.
File validation happens after selection: the app still accepts only a valid encrypted Pocket Budget backup or a supported legacy JSON backup.
Files larger than 50 MB are rejected before parsing.
Existing encrypted `.pbe` backups from Version 26 remain compatible.
iPhone import
Open Setup > Local data.
Tap Import encrypted backup.
Choose the `.pbe` file in Files.
Enter the password.
Confirm the restore.
No backup conversion is required.
Backup share correction
Version 27 now shares only the encrypted `.pbe` file. The optional Web Share `title` and `text` fields were removed because iPhone Files may materialize the shared text as a separate file.
